...
<html lang="en">
<head>
    <title>Code coverage report for src/implementation/metadata.ts</title>
    <meta charset="utf-8" />
...
<div class='wrapper'>
  <div class='pad1'>
    <h1>
      <a href="../../index.html">All files</a> / <a href="index.html">src/implementation</a> metadata.ts
    </h1>
    <div class='clearfix'>
...
<span class="fstat-no" title="function not covered" >  function generateRef() {</span>
    let bucket: string = <span class="cstat-no" title="statement not covered" >metadata['bucket'] as string;</span>
    let path: string = <span class="cstat-no" title="statement not covered" >metadata['fullPath'] as string;</span>
    let loc = <span class="cstat-no" title="statement not covered" >new Location(bucket, path);</span>
<span class="cstat-no" title="statement not covered" >    return authWrapper.makeStorageReference(loc);</span>
  }
...
export function metadataValidator(p: any) {
  ...
  for (let key in p) {
    ...
    if (key === 'customMetadata') {
      <span class="missing-if-branch" title="else path not taken" >E</span>if (!type.isObject(val)) {
